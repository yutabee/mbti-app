...
const MBTITable = () => {
  ...
  const mbtiTypes = [
    {
      ...
      decision: '過去の経験と論理に基づき、秩序立てて意思決定。感情よりも事実を重視',
    ...
    },
    {
      ...
      decision: '過去の経験と他者への影響を考慮。調和を大切にしながら現実的に判断',
    ...
    },
    {
      ...
      decision: '論理的分析と現状把握。問題解決に焦点を当て、実用的な結果を重視',
    ...
    },
    {
      ...
      decision: '論理的効率性と確立された方法。秩序と結果を重視した体系的な決定',
  ...
  const groups = [
    { id: 'all', name: 'すべて' },
    { id: '分析家（Analyst）', name: '分析家', description: '知識と可能性を探求する論理的・戦略的なタイプ（NT）' },
    { id: '外交官（Diplomat）', name: '外交官', description: '人間関係と協調性を重視する理想主義的なタイプ（NF）' },
    { id: '番人（Sentinel）', name: '番人', description: '秩序と安定を重視する実務的・責任感のあるタイプ（SJ）' },
    { id: '探検家（Explorer）', name: '探検家', description: '自由と行動を好む冒険的・柔軟なタイプ（SP）' }
  ];
  ...
  const renderElementBadges = (elements) => {
    const parts = elements.split('+');
    return (
      <div className="flex flex-wrap gap-1 mt-2">
        {parts.map((element, index) => (
          <span key={index} className="text-xs px-2 py-1 rounded-full bg-white shadow-sm">
      ...
      <div className="mt-8 bg-white rounded-xl shadow-lg overflow-hidden border border-gray-100">
        <div className={`bg-gradient-to-r ${colors.gradient} p-6 text-white`}>
          <div className="flex items-center justify-between">
            <div>
              <h3 className="text-2xl font-bold">{type.type} - {type.name}</h3>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            <span className="bg-white/20 px-3 py-1 rounded-full text-sm">{type.group}</span>
            <span className="bg-white/20 px-3 py-1 rounded-full text-sm">{type.keirsey}</span>
          </div>
        </div>
            ...
            <div className="space-y-6">
              <section>
                <h4 className="text-lg font-semibold mb-3 flex items-center">
                  <span className="inline-block w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center mr-2">🧩</span>
                  基本要素
                </h4>
                <div className="flex flex-wrap gap-2 mb-3">
                  {type.element.split('+').map((elem, i) => (
                    <div key={i} className="flex items-center bg-gray-50 rounded-lg p-2 border border-gray-100">
                      <span className="text-xl mr-2">{getElementIcon(elem)}</span>
                      <div>
                        <div className="font-medium">{elem}</div>
                        <div className="text-xs text-gray-500">{getElementName(elem)}</div>
                      </div>
                    </div>
                  ))}
                </div>
                <div className="mt-4">
                  <div className="text-sm font-medium text-gray-700 mb-1">認知機能</div>
                  <div className="flex flex-wrap gap-1">
                    {type.functions.split('-').map((func, i) => (
                      <div 
                        key={i} 
                        className={`px-2 py-1 rounded text-xs ${i === 0 ? 'bg-green-100 text-green-800' : 
              ...
              <section>
                ...
                <div className="space-y-3">
                  <div className="bg-green-50 p-3 rounded-lg border border-green-100">
                    <div className="text-sm font-medium text-green-800 mb-1">強み</div>
                    <p>{type.strengths}</p>
                  </div>
                  <div className="bg-red-50 p-3 rounded-lg border border-red-100">
                    <div className="text-sm font-medium text-red-800 mb-1">弱み</div>
                    <p>{type.weaknesses}</p>
                  </div>
                </div>
              </section>
              ...
              <section>
                ...
                <div className="space-y-3">
                  <div className="bg-gray-50 p-3 rounded-lg border border-gray-100">
                    <div className="text-sm font-medium text-gray-700 mb-1">意思決定プロセス</div>
                    <p>{type.decision}</p>
                  </div>
                  <div className="bg-amber-50 p-3 rounded-lg border border-amber-100">
                    <div className="text-sm font-medium text-amber-800 mb-1">ストレス時の特徴</div>
                    <p>{type.stress}</p>
                  </div>
                </div>
              </section>
              ...
              <section>
                ...
                <div className="space-y-3">
                  <div className="bg-blue-50 p-3 rounded-lg border border-blue-100">
                    <div className="text-sm font-medium text-blue-800 mb-1">向いている職業</div>
                    <p>{type.careers}</p>
                  </div>
                  <div className="bg-purple-50 p-3 rounded-lg border border-purple-100">
                    <div className="text-sm font-medium text-purple-800 mb-1">シャドウ機能</div>
                    <p>{type.shadow}</p>
                  </div>
                </div>
              </section>
  ...
  const renderTypeCard = (type) => {
    ...
    return (
      <div 
        key={type.type} 
        className={`border rounded-xl p-4 cursor-pointer transition-all duration-200 ${colors.bg} ${colors.border} ${colors.hover} ${expandedType?.type === type.type ? 'ring-2 ring-offset-2 ring-blue-500 shadow-md' : 'shadow-sm hover:shadow'}`}
        onClick={() => setExpandedType(expandedType?.type === type.type ? null : type)}
      >
        <div className="flex justify-between items-start">
          <div>
            <h3 className="text-lg font-bold flex items-center">
              {type.type} 
              <span className={`ml-2 text-sm py-1 px-2 rounded-full bg-white ${colors.text}`}>
                {type.name}
              </span>
            </h3>
        ...
        <div className="mt-3">
          <span className={`inline-block text-xs px-2 py-1 rounded-full bg-white/70 ${colors.text}`}>
            {type.group}
          </span>
        </div>
  ...
  const renderFunctionsExplanation = () => (
    <div className="mt-8 bg-white p-6 rounded-xl shadow-md border border-gray-100">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
        {Object.entries(functionsExplanation).map(([key, value]) => (
          <div key={key} className="border rounded-lg p-3 transition-all hover:bg-gray-50">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {filteredTypes.length > 0 ? (
          ...
          <div className="col-span-full p-8 text-center">
            ...
            <h3 className="text-xl font-medium text-gray-700">タイプが見つかりません</h3>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex gap-4 mb-8">
        <button 
          ...
          className={`px-4 py-2 rounded-lg font-medium ${activeTab === 'basic' ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700'}`}
        >
          基本
        </button>
        <button 
          onClick={() => setActiveTab('functions')} 
          className={`px-4 py-2 rounded-lg font-medium ${activeTab === 'functions' ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-700'}`}
        >
          認知機能
        </button>
      </div>
